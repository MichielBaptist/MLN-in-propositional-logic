...
{
    fstream file;
    file.open("input_for_c2d", fstream::out);
    file << "c weights";
    ...
    file.open("vtree_in_name", fstream::in);
    string vtree_in_name;
    file >> vtree_in_name;
    file.close();
    
    file.open("vtree_out_name", fstream::in);
    string vtree_out_name;
    file >> vtree_out_name;
    file.close();

    string command="./miniC2D -c input_for_c2d ";
    if(vtree_in_name != "")
    {
        command += "-v ";
    ...
    command += "_A -W > output_for_c2d";
    system(command.c_str());

	double p1 = find_wmc_in_file("output_for_c2d");
	
    file.open("input_for_c2d2", fstream::out);
    ...
    command="./miniC2D -c input_for_c2d2 ";
    if(vtree_in_name != "")
    {
        command += "-v ";
        command += vtree_in_name;
    ...
    command += "_B -W > output_for_c2d2";
    system(command.c_str());
    double p2 = find_wmc_in_file("output_for_c2d2");
