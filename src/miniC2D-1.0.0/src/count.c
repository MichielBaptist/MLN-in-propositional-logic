...
c2dWmc plus(c2dWmc n1, c2dWmc n2)
{
    c2dWmc ret;
    if(n1<n2)
    {
        ret=n1;
        if(n2-n1 <= 300) // exp(n2-n1) can be computed.
        {
            ret += log(1+exp(n2-n1));
        }
        else
    ...
    {
        ret=n2;
        if(n1-n2 <= 300) // exp(n1-n2) can be computed.
        {
            ret += log(1+exp(n1-n2));
        }
        else
...
c2dWmc times(c2dWmc n1, c2dWmc n2)
{
    return n1+n2;
}
...
c2dWmc count_vtree(VtreeManager* manager, SatState* sat_state) {
  ...
  if(sat_assert_unit_clauses(sat_state)) { //unit resolution succeeded
    ...
    if(learned_clause!=NULL) count = -1000; //cnf is inconsistent
  }
  else count = -1000; //cnf is inconsistent
...
c2dWmc var2count(Var* var) {
  ...
  else return plus(sat_literal_weight(plit), sat_literal_weight(nlit));
}
...
void count_vtree_decomposed(c2dWmc* count, Clause** learned_clause, DVtree* vtree, VtreeManager* vtree_manager, SatState* sat_state) {
  ...
  else if(l_count<=-1000) { //optimization
    *count = -1000;
    return;
  }
...
void count_vtree_shannon(c2dWmc* count, Clause** learned_clause, DVtree* vtree, VtreeManager* vtree_manager, SatState* sat_state) {
  ...
  if(sat_is_instantiated_var(var) || sat_is_irrelevant_var(var)) {
    ...
    if(*learned_clause==NULL) *count = times(*count, var2count(var)); //before: *count *= var2count(var).
    return;
  }
  ...
  /* printf("\n  pcount \t%0.3"PRIwmcS"", pcount);
  printf("\n  ncount \t%0.3"PRIwmcS"", ncount);
  printf("\n  sat_literal_weight(plit) \t%0.3"PRIwmcS"", sat_literal_weight(plit));
  printf("\n  sat_literal_weight(nlit) \t%0.3"PRIwmcS"", sat_literal_weight(nlit));
  printf("\n  times(pcount, sat_literal_weight(plit)) \t%0.3"PRIwmcS"", times(pcount, sat_literal_weight(plit)));
  printf("\n  times(ncount, sat_literal_weight(nlit)) \t%0.3"PRIwmcS"", times(ncount, sat_literal_weight(nlit)));

  printf("\n  alles \t%0.3"PRIwmcS"", plus(times(pcount, sat_literal_weight(plit)), times(ncount, sat_literal_weight(nlit)))); */

  *count = plus(times(pcount, sat_literal_weight(plit)), times(ncount, sat_literal_weight(nlit)));
